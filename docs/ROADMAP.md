...
- [x] 后端：资金曲线图 ECharts 双轴（曲线 + 日收益柱状图）
- [x] 前端：最大回撤阴影标注 + DataZoom 缩放
- [x] 叠加基准对比（沪深300 虚线）

#### 3. 基准对比 🔴 ✅ 已完成
**目标：** 回测结果有参照系

- [x] 后端：回测引擎加载基准指数日线，计算基准日收益率序列
- [x] 存储：benchmark_curve + benchmark_return + excess_return
- [x] 前端：资金曲线图上叠加基准虚线
- [ ] DB 迁移需部署后执行（ALTER TABLE 加列）

#### 4. 多股票组合回测 ✅ 已完成
**目标：** 固定资金池，多种仓位模式

- [x] 后端：三种仓位模式 — 固定金额 / 百分比 / 等权重
- [x] VisualStrategy 按仓位模式计算下单量
- [x] 前端：仓位分配选择器 + 提示说明
- [x] 修复 config 变量覆盖 bug

#### 5. 回测对比 ✅ 已完成
**目标：** 多个回测结果放在一起比较

- [x] 前端：对比页面 — 选 2-5 个回测，指标表格 + 曲线叠加
- [x] 回测列表页：API 数据 + 状态标签 + 进度条

#### 6. 导出报告 ✅ 已完成
**目标：** 一键导出回测分析结果

- [x] 前端：回测详情页「导出 CSV」按钮
- [x] CSV 包含：所有绩效指标 + 完整交易明细

#### 7. 每日信号 ✅ 已完成
**目标：** 基于策略的每日选股信号

- [x] 前端：DailySignalsView — 全策略扫描 + 按策略分组
- [x] 侧边栏导航：AlarmClock 图标
- [ ] 自动定时扫描（Celery Beat 每日 15:35）

#### 8. 多周期策略 ✅ 已完成
**目标：** 支持日线/周线/月线

- [x] 后端：MarketReplayer.resample() — OHLCV 聚合
- [x] 前端：K线周期选择器（日线/周线/月线）
- [x] API + DB: period 列支持
